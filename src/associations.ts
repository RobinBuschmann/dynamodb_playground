...
export const belongsTo = ({
    ...
    const associatedInstance = cache.get(
        createIdentifier(target.model, instance[foreignKey]),
    );
    if (associatedInstance) {
        instance[association.key] = cache.get(
            createIdentifier(target.model, instance[foreignKey]),
        );
    }
};
...
export const belongsToMany = ({
    ...
    throughInstances?.forEach(throughInstance => {
        ...
        const targetInstance = cache.get(
            createIdentifier(target.model, throughInstance[foreignKey]),
        );
        if (targetInstance) {
