...
export const getPrimaryKeyConcatValue = (schema, entry) =>
    schema.primaryKeys.map(pk => entry[pk]).join('|');

export const getPrimaryKey = ({ source }) => {
    if (source.primaryKeys.length > 1) {
        throw new Error(
            `Cannot resolve primary key of ${source.model}. Multiple primary keys aren't supported`,
        );
    }
...
export const getForeignKey = ({ source, target }) => {
    const [foreignKey] = source.references.filter(
        ref => ref.model === target.model,
    );
    return foreignKey;
};
